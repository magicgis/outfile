	...
	_setting = {
		...
		callback: {
			beforeCheck:function(treeId,treeNode){
				var checedkNodes=data.getZTreeTools(treeId).getCheckedNodes(true);
				if(checedkNodes&&checedkNodes.length>=999&&treeNode.checked===false){
	...
	_view = {
		checkNodeRelation: function(setting, node) {
			...
			if (setting.check.chkStyle == r.STYLE) {
				...
				if (node[checkedKey] && (!node[childKey] || node[childKey].length==0 || setting.check.chkboxType.Y.indexOf("s") > -1)) {
					continueCheck=view.setSonNodeCheckBox(setting, node, true);
					if(continueCheck===false){
						if(!window.message){
							alert("选中记录数不能超过1000条，超过1000条记录只能选中前1000条记录！");
						}else{
				...
				if (!node[checkedKey] && (!node[childKey] || node[childKey].length==0 || setting.check.chkboxType.N.indexOf("s") > -1)) {
					continueCheck=view.setSonNodeCheckBox(setting, node, false);
					if(continueCheck===false){
						if(!window.message){
							alert("选中记录数不能超过1000条，超过1000条记录只能选中前1000条记录！");
						}else{
				...
				if (node[checkedKey] && setting.check.chkboxType.Y.indexOf("p") > -1) {
					continueCheck=view.setParentNodeCheckBox(setting, node, true);
					if(continueCheck===false){
						if(!window.message){
							alert("选中记录数不能超过1000条，超过1000条记录只能选中前1000条记录！");
						}else{
				...
				if (!node[checkedKey] && setting.check.chkboxType.N.indexOf("p") > -1) {
					continueCheck=view.setParentNodeCheckBox(setting, node, false);
					if(continueCheck===false){
						if(!window.message){
							alert("选中记录数不能超过1000条，超过1000条记录只能选中前1000条记录！");
						}else{
		...
		repairAllChk: function(setting, checked) {
			if (setting.check.enable && setting.check.chkStyle === consts.checkbox.STYLE) {
				...
				for (var i = 0, l = root[childKey].length; i<l ; i++) {
					...
					continueCheck=view.setSonNodeCheckBox(setting, node, checked);
					if(continueCheck===false){
						if(!window.message){
							alert("选中记录数不能超过1000条，超过1000条记录只能选中前1000条记录！");
						}else{
		...
		setParentNodeCheckBox: function(setting, node, value, srcNode) {
			...
			if (node.parentTId) {
				...
				if (pSign) {
					var checedkNodes=data.getZTreeTools(setting.treeId).getCheckedNodes(true);
					if(checedkNodes&&checedkNodes.length>=999){
						return false;
					}
