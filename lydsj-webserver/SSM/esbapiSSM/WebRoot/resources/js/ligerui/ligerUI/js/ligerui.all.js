    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        doMax: function (slide)
        {
            ...
            if (slide)
            {
                ...
                g.dialog.content.animate({ height: height - g._borderY - g.dialog.buttons.outerHeight()-20 }, p.slide, function ()
                {
                    g._onReisze();
                });
            }
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        checkAllNodes:function(checked){
        	...
        	$("li", g.tree).each(function (){
                ...
                if(checked){
                	g.selectNode(treeitem);
                }else{
                	g.cancelSelect(treeitem);
                }
             });
        },
        ...
        {
        	//modify by chenguojun 根据节点的最长文本决定宽度
            var g = this, p = this.options;
            var nodes = g.nodes;
            ...
            if(nodes){
            	for(var i in nodes){
            		var node = nodes[i];
            		treeWidth = node.text.length * fontsize;
            	}
            	treeWidth += g.maxOutlineLevel * fontsize;
            }
