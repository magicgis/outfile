...
@RestController
@RequestMapping("/v2/qskydpm")
public class QskydpmController extends BaseController {
	@Resource
	private TjKydfxMonthlyService tjKydfxMonthlyService;
	
	@RequestMapping("/{year_id}/{area_range}/{id_type}/{id}")
	public HashMap<String, Object> getKydfxTop5( @PathVariable Integer year_id,  @PathVariable Integer area_range, @PathVariable Integer id) {
		...
		String[] title = { "排名", "地点", "占比（%）" };
		List<KydfxTop5Data> list = new ArrayList<>();
		list = tjKydfxMonthlyService.getQskydpm(year_id, area_range);
