...
public class ConvertPdf {
    ...
    public ConvertPdf(String fileString) {
        ini(fileString);
        ...
        if (!swfFile.exists()) {
            if (pdfFile.exists()) {
                if (environment == 1) {// windows环境处理
                    try {
                        Process p = r.exec("G:/SWFTools/pdf2swf.exe "+ pdfFile.getPath() +" -s flashversion=9"+ " -o "+ swfFile.getPath() + " -T 9");
    ...
    {
    	try {
    		ConvertPdf d=new ConvertPdf("D:\\CRM\\Files\\mis\\excel\\sampleoutput\\ORD-18001124_SupplierOrder_testuser_20180331_110730.xls");
            d.conver();
		} catch (Exception e) {
			e.printStackTrace();
		}
