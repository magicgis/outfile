 ...
 *
 */
@Service("quoteProfitStatementExcelGenerator")
public class QuoteProfitStatementExcelGenerator extends ExcelGeneratorBase {
	@Resource
...
//	}
	...
	protected String fetchTemplateFileName(String currencyId) {
		return File.separator + "exceltemplate" + File.separator + "QuoteProfit.xls";
	}
	...
	protected void writeData(POIExcelWorkBook wb,String ywId) {
		...
		List<ProfitStatementVo> cqelist = clientQuoteService.findQuoteProfitStatement(ywId);
		...
		for (ProfitStatementVo profitStatementVo : cqelist) {
			List<ClientQuoteElementVo> List = clientQuoteService
					.findQuoteDataByCieId(profitStatementVo.getClientInquiryElementId());
		...
		Double	cqex=cqelist.get(0).getCqExchangeRate();
		Double sqex=cqelist.get(0).getSqExchangeRate();
		String cqCurrency=cqelist.get(0).getCqCurrencyValue();
		String sqCurrency=cqelist.get(0).getSqCurrencyValue();
...
//				l.getQuoteBasePrice();
				...
				Cell boldPriceCellTmpl = supplierSheetLastRow.getCell(5);
				...
				if(cqCurrency.equals("美元")&&sqCurrency.equals("美元")){
...
//					supplierCell.setCellStyle(supplierQuotePriceCellTmpl.getCellStyle());
					...
					if (sqe != null) {
						...
						BigDecimal sqePriceT = sqePrice.multiply(quoteAmount.subtract(storageAmount));
			...
			for (int j = 0; j < 5; j++) {
				...
				if(j!=3){
					cell.setCellStyle(cellStyle);
						...
						cell.setCellStyle(totalRowTmpl.getCell(8+j).getCellStyle());
					}
//				switch (j) {
//				case 0:
...
//				}
				if(j==0){
					...
					cell.setCellFormula("(" + clientQuotePriceColumn + (totalRow.getRowNum() + 1) + "-"
							+ quoteTotalPriceColumn + (totalRow.getRowNum() + 1) + "-" + storagePriceColumn
							+ (totalRow.getRowNum() + 1) + ")/" + clientQuotePriceColumn
							+ (totalRow.getRowNum() + 1));
				}else if(j==4){
