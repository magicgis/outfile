...
@Service("supplierArrivalListExcelGenerator")
public class SupplierArrivalListExcelGenerator extends ExcelGeneratorBase {
	...
	protected String fetchMappingKey() {
		return "SupplierArrivalListExcel";
	}

	@Override
	protected String fetchTemplateFileName(String currencyId) {
		return File.separator+"exceltemplate"+File.separator+"SupplierArrivalList.xls";
	}
				...
				if (cell != null && cell.getCellType() == Cell.CELL_TYPE_STRING) {
					...
					if (str.startsWith("$")) {//拿到以$开头的变量
						...
						if(key.equals("date")){//当变量等于这个的时候，就把数据写进去
							String importpackagenumber=importPackage.get(0).getImportNumber();
							 value =importpackagenumber.substring(0, 8)+"到货清单";
						}
	...
	protected String fetchOutputFileName() {
		...
		Date now = new Date();
		 ...
		 return number+"_"+"SupplierArrivalList"+"_"+this.fetchUserName()+"_"+format.format(now);
	}

	@Override
	protected String fetchYwTableName() {
		return "Supplier_arrival_list";
	}
