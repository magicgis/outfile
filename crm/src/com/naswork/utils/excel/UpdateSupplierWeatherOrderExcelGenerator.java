 ...
 *
 */
@Service("updateSupplierWeatherOrderExcelGenerator")
public class UpdateSupplierWeatherOrderExcelGenerator extends ExcelGeneratorBase {
	...
	protected String fetchTemplateFileName(String currencyId) {
		return File.separator+"exceltemplate"+File.separator+"UpdateSupplierWeatherOrder.xlsx";
	}
	...
	protected void writeData(POIExcelWorkBook wb,String ywId) {
		...
		List<ClientOrderElementVo>  list=supplierOrderElementDao.SupplierWeatherOrderData(clientQuote);
...
//						elementCell.setCellStyle(cell.getCellStyle());
						if (!StringUtils.isEmpty(cellKeys[cellnum
							...
							if(key.equals("ORDER_NUMBER")){
								...
								cellNumber=cellnum;
								String[] priceColumns = getColumnLabels(cellNumber+3);
								String[] amountColumns = getColumnLabels(cellNumber+4);
								...
								Cell totalCell = elementRow.createCell(cellNumber+4);
								totalCell.setCellFormula(priceCell+"*"+amountCell);
								this.write(elementCell, value);
							}else if (key.equals("WEATHER_ORDER_NUMBER")) {
								Integer supplierQuoteElementId=elementVo.getWeatherSupplierQuoteElementId();
								SupplierQuoteElement element=supplierQuoteElementDao.selectByPrimaryKey(supplierQuoteElementId);
								Object value = element.getSupplierInquiryQuoteNumber()+"/"+supplierQuoteElementId;
								this.write(elementCell, value);
							}else if (key.equals("WEATHER_ORDER_CODE")) {
								Integer supplierQuoteElementId=elementVo.getWeatherSupplierQuoteElementId();
								SupplierQuoteElement element=supplierQuoteElementDao.selectByPrimaryKey(supplierQuoteElementId);
								Object value = element.getCode();
								this.write(elementCell, value);
							}else if (key.equals("WEATHER_ORDER_PRICE")) {
								Object value = elementVo.getWeatherPrice();
								this.write(elementCell, value);
							}else if (key.equals("WEATHER_ORDER_AMOUNT")) {
								Object value = elementVo.getWeatherAmount();
								this.write(elementCell, value);
...
//			String[] textList1 = new String[] { "其他", "香港仓库","美国仓库"};
//			String[] textList2 = new String[] { "FEDEX", "DHL",
//					"UPS", "EMS","顺丰快递", "德邦物流","天地华宇","空运","其他物流","Dropship FedEx","Dropship DHL","Dropship other","ForWarder"};
//			String[] textList3 = new String[]{ "FN", "NE", "NS", "SV", "OH", "AR","Exchange" };
//			String[] textList4 = new String[]{ "OEM COC", "FAA 8130-3",
//					"EASA Form One", "Vendor COC","MFR CERT", "CAAC","FAA+CAAC","EASA+CAAC","TCCA Form one","Other" };
...
//					lastRowNum + list.size() - 1,supplierCode.size()));
		...
		for (int cellnum = 7; cellnum <= lastCellNum; cellnum++) {
...
//}
	...
	protected String fetchYwTableName() {
		return "update_supplier_weather_order";
	}
	...
	protected String fetchMappingKey() {
		return "UpdateSupplierWeatherOrderExcel";
	}
