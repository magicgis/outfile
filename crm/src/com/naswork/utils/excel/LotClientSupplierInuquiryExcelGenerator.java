...
@Service("lotClientSupplierInuquiryExcelGenerator")
public class LotClientSupplierInuquiryExcelGenerator extends ExcelGeneratorBase{
	@Resource
	...
	protected String fetchTemplateFileName(String currencyId) {
		return File.separator+"exceltemplate"+File.separator+"LotClientSupplierInquiry.xlsx";
	}
	...
	protected void writeData(POIExcelWorkBook wb,String ywId) {
		...
		page.put("ids", ywId);
		List<ClientInquiryElement> inquiryElements = clientInquiryElementService.getLotsInquiryElements(page);
		...
		if (inquiryElements != null
				&& inquiryElements.size() > 0) {
			for (int i = 0; i < inquiryElements.size(); i++) {
				//lastRowNum = sheet.getLastRowNum();
				Row elementRow = sheet.createRow(lastRowNum + i);
				for (int cellnum = firstCellNum; cellnum <= lastCellNum; cellnum++) {
					Cell cell = lastRow.getCell(cellnum);
					Cell elementCell = elementRow.createCell(cellnum);
					ClientInquiryElement manageElementVo = inquiryElements.get(i);
					if (cell != null) {
						...
						if (!StringUtils.isEmpty(cellKeys[cellnum
							...
							if (key.equals("ITEM")) {
								...
								Object value = manageElementVo.getQuoteNumber();
								this.write(elementCell, value);
							}else if (key.equals("TYPE")) {
								Object value = manageElementVo.getTypeCode();
								this.write(elementCell, value);
							}else if (key.equals("CSN")) {
								Object value = manageElementVo.getCsn();
								this.write(elementCell, value);
								...
								List<String> cages = tPartService.getCageCodeByShortCode(clientInquiryService.getCodeFromPartNumber(manageElementVo.getPartNumber()));
								if (cages.size() > 0) {
								...
								Object value = manageElementVo.getId();
								this.write(elementCell, value);
							}else if (key.equals("SHORT_PART_NUMBER")) {
								Object value = clientInquiryService.getCodeFromPartNumber(manageElementVo.getPartNumber());
								this.write(elementCell, value);
							}
						}
					}
	...
	protected String fetchOutputFileName() {
		...
		return "LotClientSupplierInquiry"+"_"+this.fetchUserName()+"_"+format.format(now);
	}
	...
	protected String fetchYwTableName() {
		return "lot_client_supplier_inquiry";
	}
	...
	protected String fetchMappingKey() {
		return "LotClientSupplierInuquiryExcel";
	}
