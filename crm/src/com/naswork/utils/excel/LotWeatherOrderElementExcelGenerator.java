...
@Service("lotWeatherOrderElementExcelGenerator")
public class LotWeatherOrderElementExcelGenerator extends ExcelGeneratorBase{
	@Resource
	...
	protected void writeData(POIExcelWorkBook wb,String ywId) {
		...
		page.put("ids", ywId);
		List<ClientOrderElementVo> orderElements = clientWeatherOrderElementService.getElementByIds(page);
		...
		if (orderElements != null
			...
			for (int i = 0; i < orderElements.size(); i++) {
				...
				for (int cellnum = firstCellNum; cellnum <= lastCellNum; cellnum++) {
					...
					ClientOrderElementVo manageElementVo = orderElements.get(i);
					if (cell != null) {
						...
						if (!StringUtils.isEmpty(cellKeys[cellnum
							...
							if(key.equals("PART_NUMBER")){
								...
								Object value = clientInquiryService.getCodeFromPartNumber(manageElementVo.getInquiryPartNumber());
								this.write(elementCell, value);
								...
								List<String> cages = tPartService.getCageCodeByShortCode(clientInquiryService.getCodeFromPartNumber(manageElementVo.getInquiryPartNumber()));
								if (cages.size() > 0) {
	...
	protected String fetchOutputFileName() {
		...
		return "LotOrderElement"+"_"+this.fetchUserName()+"_"+format.format(now);
	}
