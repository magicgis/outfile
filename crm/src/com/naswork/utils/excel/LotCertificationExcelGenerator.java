...
@Service("lotCertificationExcelGenerator")
public class LotCertificationExcelGenerator extends ExcelGeneratorBase {
			...
			for (int cellnum = row.getFirstCellNum(); cellnum <= row
				...
				if (cell != null && cell.getCellType() == Cell.CELL_TYPE_STRING) {
					...
					if (str.startsWith("$")) {
						...
						else if(key.equals("SHELF_LIFE1")){
						...
						if(!"".equals(manDate)){
							...
							else if(key.equals("SHELF_LIFE2")){
							...
							if(key.equals("SOURCE_ORDER_NUMBER3")){
							...
							else if(key.equals("SHELF_LIFE3")){
								if (ipelist3.getShelfLife() != null) {
									...
									Row row1 = sheet.getRow(48);
									Cell cell1 = row1.getCell(1);
									this.write(cell1, "");
									value ="";
									Row row11 = sheet.getRow(49);
									Cell cell11 = row11.getCell(1);
									this.write(cell11, "");
								}
							}
							...
							else if(key.equals("SHELF_LIFE4")){
...
//		partNumber=	partNumber.replace("\\", "＼");
	...
	protected String fetchYwTableName() {
		return "export_package_instructions";
	}
