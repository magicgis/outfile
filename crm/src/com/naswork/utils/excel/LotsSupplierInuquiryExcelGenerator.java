...
@Service("lotsSupplierInuquiryExcelGenerator")
public class LotsSupplierInuquiryExcelGenerator extends ExcelGeneratorBase{
	@Resource
	...
	protected String fetchTemplateFileName(String currencyId) {
		return File.separator+"exceltemplate"+File.separator+"SupplierInquiry.xls";
	}
	...
	protected void writeData(POIExcelWorkBook wb,String ywId) {
		...
		String[] inquiryIds = index[0].split(",");
		//for (int j = 0; j < inquiryIds.length; j++) {
			PageModel<ManageElementVo> page=new PageModel<ManageElementVo>();
			SupplierInquiry supplierInquiry = supplierInquiryService.selectByPrimaryKey(new Integer(inquiryIds[new Integer(index[1])]));
			number = supplierInquiry.getQuoteNumber();
			Supplier supplier = supplierInquiryService.getSupplier(supplierInquiry.getSupplierId());
			page.put("supplierInquiryId", new Integer(inquiryIds[new Integer(index[1])]));
			List<ManageElementVo> elementList = supplierInquiryService.Elements(new Integer(inquiryIds[new Integer(index[1])]));
				...
				for (int cellnum = row.getFirstCellNum(); cellnum <= row
					...
					if (cell != null && cell.getCellType() == Cell.CELL_TYPE_STRING) {
						...
						if (str.startsWith("$")) {
							...
							Object value = null;
							if (key.equals("SUPPLIER_CONTACT_NAME")) {
			...
			if (elementList != null
				...
				for (int i = 0; i < elementList.size(); i++) {
					...
					for (int cellnum = firstCellNum; cellnum <= lastCellNum; cellnum++) {
						...
						ManageElementVo manageElementVo = elementList.get(i);
						if (cell != null) {
	...
	private HSSFDataValidation getCertificationValidation(int firstRow,
		...
		String[] textList = new String[] { "OEM COC", "FAA 8130-3",
				"EASA Form One", "Vendor COC","MFR CERT", "CAAC","FAA+CAAC","EASA+CAAC","TCCA Form one","FAA Dual Release","EASA Dual Release","FAA+EASA","Dual Release","Other" };
		DVConstraint constraint = DVConstraint
				.createExplicitListConstraint(textList);
		return new HSSFDataValidation(regions, constraint);
	...
	protected String fetchOutputFilePath() {
		return FileConstant.UPLOAD_REALPATH+File.separator+FileConstant.UPLOAD_FOLDER+File.separator+"sampleoutput";
	...
	protected String fetchOutputFileName() {
		...
		return number+"_"+"SupplierInquiry"+"_"+this.fetchUserName()+"_"+format.format(now);
	}
	...
	protected String fetchMappingKey() {
		return "LotsSupplierInuquiryExcel";
	}
