...
@Service("lotImportPackageElementExcelGenerator")
public class LotImportPackageElementExcelGenerator extends ExcelGeneratorBase{
	@Resource
	...
	protected void writeData(POIExcelWorkBook wb,String ywId) {
		...
		page.put("ids", ywId);
		List<ImportPackageElement> importElements = importpackageElementService.exportLotsExcelElement(page);
		...
		if (importElements != null
			...
			for (int i = 0; i < importElements.size(); i++) {
				...
				for (int cellnum = firstCellNum; cellnum <= lastCellNum; cellnum++) {
					...
					ImportPackageElement manageElementVo = importElements.get(i);
					if (cell != null) {
	...
	protected String fetchOutputFileName() {
		...
		return "LotImportElement"+"_"+this.fetchUserName()+"_"+format.format(now);
	}
