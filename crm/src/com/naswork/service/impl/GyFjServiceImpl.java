...
public class GyFjServiceImpl implements GyFjService{
	...
	@Override
	public void searchPageForHis(PageModel<GyFj> page, String where, GridSort sort) {
		if(!"".equals(where) && where != null){
			String businessKey = page.getString("businessKey");
			String ywid = businessKey.substring(businessKey.lastIndexOf(".") + 1, businessKey.length());
			String tableName = businessKey.split("\\.")[0];
			where = where + " and " + page.get("append");
			page.put("where", where);
		}else {
			page.put("where", page.get("append"));
		}
		if(sort!=null){
			sort.setName(ConvertUtil.toDBName(sort.getName()));
			page.put("orderby", ConvertUtil.convertSort(sort));
	...
	public List<GyFj> getQuoteAttach(PageModel<String> page){
		return gyFjDao.getQuoteAttach(page);
	}
