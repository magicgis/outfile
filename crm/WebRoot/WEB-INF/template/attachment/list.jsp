...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			toolbar =$("#toolbar").ligerToolBar({
				items:[{
					...
					click : function() {
						var ids = PJ.grid_getMutlSelectedRowkey(grid);
						if(ids && ids!=""){
							PJ.addTab("查看PDF", '<%=path%>/fj/toViewPdf?ids='+ids);
			...
			grid = PJ.grid("list1", {
				rowNum: 10,
				url:'<%=path%>/fj/list/data?${data}'+'&roleId='+'${roleId}',
  ...
  <body>
   		...
   		<!-- 附件上传 -->
   		<div class="table-box" style="display: none;height: 130px" id="uploadDiv">
   			<form:row>
    			<form:column>
    		...
    		<form:row>
    			<form:column>
    				<form:left>附件类型：</form:left>
    				<form:right>	
    				<select id="fileType" name="fileType"  class="text">
