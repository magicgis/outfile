...
<script type="text/javascript">
	...
	$(function() {
			...
			items : [
						{
							...
							click: function(){
									...
									$.ajax({
										type: "POST",
										dataType: "json",
										url:'<%=path%>/storage/exportpackage/lotsExcel?location='+location+'&id='+id,
										success:function(result){
											if(result.success){
												...
												var businessKey = 'export_package_instructions.id.'+id+location+'.LotCertificationExcel';
												PJ.excelDiaglog({
		...
		grid = PJ.grid("list1", {
			rowNum: 100,
			url:'<%=path%>/storage/exportpackage/locationList?id='+${id},
			...
			colNames :["位置"],
			colModel :[
			           PJ.grid_column("location", {sortable:true,width:200,align:'left',key:true})
			           ]
		});
		...
		$(document).keypress(function(e) {
		    ...
		    if(e.which == 13) {
				...
				if(where != "" && where != null){
					PJ.grid_search(grid,'<%=path%>/storage/exportpackage/locationList?id='+${id});
				}
		    }  
		}); 
			...
			<div class="search-box">
				<form:row columnNum="3">
					...
					<form:column>
						...
						<form:right><p><input id="quoteNumber" name="quoteNumber" class="text" alias="ipe.LOCATION" oper="cn"/></p></form:right>
					</form:column>
					<form:column >
