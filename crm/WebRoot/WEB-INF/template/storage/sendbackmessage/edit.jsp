...
<script type="text/javascript">
	$(function(){
		$.ajax({
			url:'<%=path%>/storage/sendbackmessage/statusList?id='+'${sendBackMessage.manageStatus}',
			type: "POST",
			dataType: "json",
...
<div position="center" title="新增库存">
	...
	<input class="hide" name="id" id="id" value="${sendBackMessage.id}"/>
	<table>
		<tr>
			<td>数量</td>
			<td><input name="amount" id="amount" class="text" value="${sendBackMessage.amount }"/></td>
		</tr>
		<tr>
			<td>SN</td>
			<td><input name="serialNumber" id="serialNumber" class="text" value="${sendBackMessage.serialNumber }"/></td>
		</tr>
		<tr>
			<td>位置</td>
			<td><input name="location" id="location" class="text" value="${sendBackMessage.location }"/></td>
		</tr>
		<tr>
			<td>状态</td>
			<td>
		...
		<tr>
			<td>备注</td>
			<td><textarea rows="10" cols="90" name="remark">${sendBackMessage.remark }</textarea></td>
		</tr>
	</table>
	</form>
</div>	
