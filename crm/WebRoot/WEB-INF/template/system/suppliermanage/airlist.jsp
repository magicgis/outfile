...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#toolbar").ligerToolBar({
			items : [  {
				...
				click : function(){
					...
					PJ.topdialog(iframeId, '新增供应商机型', 
				 			'<%=path%>/suppliermanage/toAddSupplierAir?supplierId='+'${supplierId}',
								function(item, dialog){
										 var postData=top.window.frames[iframeId].getFormData();
										 $.ajax({
											    url: '<%=path%>/suppliermanage/addSupplierAir',
												data: postData,
												type: "POST",
												loading: "正在处理...",
			       ...
			       ,{
						...
						click : function(){
							...
							var id = ret["id"];
												 ...
												 $.ajax({
													    url: '<%=path%>/suppliermanage/deleteSupplierAir?supplierId='+'${supplierId}'+'&airId='+id,
		...
		grid = PJ.grid("list1", {
			rowNum: 20,
			url:'<%=path%>/suppliermanage/airlistData?supplierId='+'${supplierId}',
			...
			colModel :[PJ.grid_column("id", {key:true,hidden:true}),
			           PJ.grid_column("value",{sortable:true,width:150})
			           ]
		});
...
<input type="text" class="hide" name="sex" id="sex" value="">
	<div id="layout1">
		<div position="center" title="供应商机型管理">
		<div id="toolbar"></div>
