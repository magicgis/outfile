...
<script type="text/javascript">
	...
	$(function() {
		...
		var currency,confition,certification;
		...
		$("#toolbar").ligerToolBar({
			items : [{
						...
						click: function(){
							...
							PJ.topdialog(iframeId, '新增', '<%=path%>/system/staticprice/toAddForSupplier',
									function(item,dialog){
										 var postData=top.window.frames[iframeId].getFormData();	 							
										 PJ.ajax({
												url: '<%=path%>/system/staticprice/addWithPageForSupplier',
												data: postData,
												type: "POST",
												loading: "正在处理...",
							...
							click: function(){
								...
								PJ.confirm("确定删除?", function(yes){
									if(yes){
										PJ.showLoading("正在处理...");
										PJ.ajax({
											url: '<%=path%>/system/staticprice/supplierDelete?id='+id,
											type: "POST",
											success: function(result){
													PJ.hideLoading();
													if(result.success){
		...
		grid = PJ.grid("list1", {
			rowNum: 20,
			url:'<%=path%>/system/staticprice/supplierListData',
			width : PJ.getCenterWidth(),
			height : PJ.getCenterHeight()-102,
			autoSrcoll:true,
			shrinkToFit:true,
			sortname : "ssqp.update_timestamp",
			sortorder : "desc",
			inLineEdit: true,
			editurl:'<%=path%>/system/staticprice/editSupplierQuote',
			...
			onSelectRow:function(rowid,result){
				...
				var id = ret["id"];
				$.ajax({
					url: '<%=path%>/system/staticprice/currencyTypeForSupplier?id='+id,
					type: "POST",
					loading: "正在处理...",
					success: function(result){
				...
				$.ajax({
					url: '<%=path%>/system/staticprice/findConditionForSupplier?id='+id,
					type: "POST",
					loading: "正在处理...",
					success: function(result){
							//var obj = eval(result.message)[0];
							if(result.success){
								confition = result.message;
								/* for(var i in obj){
									currency = currency + obj[i].currency_id + ":" + obj[i].currency_value + ";";
								} */
							}
				...
				$.ajax({
					url: '<%=path%>/system/staticprice/findCertificationForSupplier?id='+id,
					type: "POST",
					loading: "正在处理...",
					success: function(result){
							//var obj = eval(result.message)[0];
							if(result.success){
								certification = result.message;
								/* for(var i in obj){
									currency = currency + obj[i].currency_id + ":" + obj[i].currency_value + ";";
								} */
							}
			...
			colNames :["id","客户","件号","价格","币种","周期","状态","证书","年份","更新时间"],
			colModel :[PJ.grid_column("id", {key:true,hidden:true,editable:true}),
			           ...
			           PJ.grid_column("leadTime", {sortable:true,width:130,align:'left',editable:true}),
			           PJ.grid_column("conditionValue", {sortable:true,width:80,align:'left',editable:true,
			        	   edittype:"select",formatter:"",
		        		   editoptions:{value:
								function(){
										return confition;        			   
			           ...
			           PJ.grid_column("certificationValue", {sortable:true,width:120,align:'left',editable:true,
			        	   edittype:"select",formatter:"",
		        		   editoptions:{value:
								function(){
		...
		$(document).keypress(function(e) {
			// 回车键事件  
		    if(e.which == 13) {
		    	PJ.grid_search(grid,'<%=path%>/system/staticprice/supplierListData');
		    }  
		}); 
		...
		$("#submit").click(function(){
			var url = '<%=path%>/system/staticprice/uploadExcelForSupplier?editType='+ getFormData().toString();
			PJ.showLoading("上传中....");
			//批量新增来函案源
	   	 	$.ajaxFileUpload({  
	            url: url,
					...
					<div class="search-box">
						<form:row columnNum="5">
							...
							<form:column>
								...
								<form:right><p>
												<select id="supplier" name="supplier" alias="s.id" oper="eq">
