...
<script type="text/javascript">
	...
	$(function() {
			...
			items : [ 
				...
				click: function(){
					var iframeId="ideaIframe1";
						PJ.topdialog(iframeId, '新增可询价供应商', '<%=path%>/system/airmanage/toAddSupplier',
								function(item, dialog){
									var postData=top.window.frames[iframeId].getFormData();
									if(postData){
										$.ajax({
											url: '<%=path%>/system/airmanage/addSupplier?airId='+'${id}',
											data: postData,
											type: "POST",
											loading: "正在处理...",
			       ...
			       ,{
						...
						click : function(){
							...
							var id = ret["id"];
												 $.ajax({
													    url: '<%=path%>/system/airmanage/deleteSupplier?supplierId='+id+'&airId='+'${id}',
		...
		grid = PJ.grid("list1", {
			rowNum: 20,
			url:'<%=path%>/system/airmanage/airSupplierlistData?id='+'${id}',
