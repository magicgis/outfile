...
<script type="text/javascript">
	...
	$(function() {
			...
			items : [
						{
							...
							click: function(){
								var iframeId="ideaIframe1";
									PJ.topdialog(iframeId, '新增机型', '<%=path%>/system/airmanage/toAdd',
											function(item, dialog){
												var postData=top.window.frames[iframeId].getFormData();
												var nullAble=top.window.frames[iframeId].validate();
												if(nullAble){
													$.ajax({
														url: '<%=path%>/system/airmanage/saveAir',
														data: postData,
														type: "POST",
														loading: "正在处理...",
						 ...
						 },
						 {
								...
								click: function(){
									...
									var iframeId="airSupplierframe";
										PJ.topdialog(iframeId, '系统配置-可询价供应商', '<%=path%>/system/airmanage/airSupplier?id='+id,
						 ...
						 },
						 {
								id:'add',
								icon : 'add',
								text : 'stockmarket供应商关联',
								click: function(){
									var ret = PJ.grid_getSelectedData(grid);
									var id = ret["id"];
									var iframeId="airSupplierframe";
										PJ.topdialog(iframeId, '系统配置-stockmarket供应商关联列表', '<%=path%>/system/airmanage/airSupplierForStockMarket?id='+id,
												undefined,function(item,dialog){dialog.close();}, 1000, 500, true);
		...
		grid = PJ.grid("list1", {
			rowNum: 20,
			url:'<%=path%>/system/airmanage/airManage',
