...
<script type="text/javascript">
	...
	$(function() {
			...
			items : [
						{
							...
							click: function(){
								var iframeId="ideaIframe1";
									PJ.topdialog(iframeId, '新增机型', '<%=path%>/system/airmanage/toAddAirForStockMarket',
											function(item, dialog){
												var postData=top.window.frames[iframeId].getFormData();
												var nullAble=top.window.frames[iframeId].validate();
												if(nullAble){
													$.ajax({
														url: '<%=path%>/system/airmanage/saveAirForStockMarket',
														data: postData,
														type: "POST",
														loading: "正在处理...",
		...
		grid = PJ.grid("list1", {
			rowNum: 20,
			url:'<%=path%>/system/airmanage/airManageForStockMarket',
