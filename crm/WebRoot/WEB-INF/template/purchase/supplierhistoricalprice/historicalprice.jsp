...
$(function(){
	...
	grid = PJ.grid("list1", {
		rowNum: 10,
		url:'<%=path%>/order/historicalorderprice/supplierOrderList',
		...
		sortname : "hop.`year`",
		sortorder : "desc",
		colNames :["id","bsn","供应商","件号","描述","年份","数量","价格"],
		colModel :[
		           	PJ.grid_column("id", {key:true, sortable:true, width:50}),
		           	PJ.grid_column("bsn", {sortable:false,width:80,hidden:true}),
		        	PJ.grid_column("code", {sortable:false,width:80}),
		           	PJ.grid_column("partNum", {sortable:false,width:120}),
		           	PJ.grid_column("partName", {sortable:false,width:40}),	
		        	PJ.grid_column("year", {sortable:false,width:100}),
		        	PJ.grid_column("amount", {sortable:false,width:40}),
		           	PJ.grid_column("price", {sortable:false,width:80})
		           ]
	});
	...
	$(document).keypress(function(e) {
		// 回车键事件  
	    if(e.which == 13) {
	    	PJ.grid_search(grid,'<%=path%>/order/historicalorderprice/supplierOrderList');
	    }  
	}); 
	...
	$("#submit").click(function(){
		var url = '<%=path%>/order/historicalorderprice/uploadExcelForSupplier?editType='+ getFormData().toString();
		PJ.showLoading("上传中....");
		//批量新增来函案源
   	 	$.ajaxFileUpload({  
            url: url,
				...
				<div class="search-box">
					<form:row columnNum="5">
						<form:column>
							...
							<form:right><p><input id="partNumber" class="text" type="text" name="tp.part_num"/> </p></form:right>
						</form:column>
						<form:column>
