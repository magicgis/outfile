...
$(function(){
					 	...
					 	PJ.topdialog(iframeId, '退货','<%=path%>/importpackage/returnGoodsPage?id='+id,
							function(item, dialog){
									 ...
									 var nullAble=top.window.frames[iframeId].validate();
										if(nullAble){
									 $.ajax({
										    url: '<%=path%>/importpackage/abnormalpartreturnGoods?importpackageelementid='+id,
											data: postData,
											type: "POST",
											loading: "正在处理...",
	...
	grid = PJ.grid("list1", {
		...
		//shrinkToFit:false,
	colNames :["id","明细id","件号","数量","描述","备注","供应商","联系人","电话","手机","地址","流程图","审批意见"],
	colModel :[PJ.grid_column("id", {key:true,hidden:true,editable:true}),
	           ...
	           PJ.grid_column("code", {editable:true,width:90}),
	           PJ.grid_column("contactName", {sortable:true,width:90,align:'left'}),
	           PJ.grid_column("phone", {sortable:true,width:150,align:'left',editable:true}),
	           PJ.grid_column("mobile", {sortable:true,width:160,align:'left',editable:true}),
	           PJ.grid_column("address", {sortable:true,width:300,align:'left'}),
