...
<script type="text/javascript">
	...
	$(function() {
		...
		PJ.ajax({
			 url: '<%=path%>/clientquote/list/dynamicColNames?clientquoteid='+'${clientquoteid}'+'&type='+'weather'+'&clientOrderId='+'${clientOrderId}',
			 dataType:'json',
	            loading: "正在加载...",
	            success: function(result) {
	            	var colNames = ["id","选中供应商","供应商价格","序号","件号", "描述","运费", "订单数量", "库存数量","供应商订单数量"];
	            	var colModel = [PJ.grid_column("id", {sortable:true,hidden:true}),
	        						...
	        						PJ.grid_column("supplierOrderAmount", {sortable:true,width:90})];
					...
					grid = PJ.grid("list", {
	    				rowNum: -1,
	    				url:'<%=path%>/clientquote/list/data?clientOrderId='+'${clientOrderId}'+'&type='+'weather',
	    				autoSrcoll:true,
	    				shrinkToFit:false,
		...
		$(window).resize(function() {
			...
			if(display=="block"){
				grid.setGridHeight(PJ.getCenterHeight()-282);
			}else{
	...
	function Excel(){
 		var clientOrderId =${clientOrderId};
			...
			var businessKey = 'profit_statement.id.'+clientOrderId+'.profitExcel'
			PJ.excelDiaglog({
