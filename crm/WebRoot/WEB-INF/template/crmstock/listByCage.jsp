		 ...
		 },
		 {
				...
				click: function(){
					...
					var iframeId="ideaIframe2";
						PJ.topdialog(iframeId, '修改商品库', '<%=path%>/stock/search/toUpdateCageCode?msn='+msn,
								function(item, dialog){
									var postData=top.window.frames[iframeId].getFormData();
									if(postData!=null){
										 $.ajax({
												url: '<%=path%>/stock/search/editCageCode',
												data: postData,
												type: "POST",
												loading: "正在处理...",
												success: function(result){
				 ...
				 },
				 {
						...
						click: function(){
							...
							var iframeId="ideaIframe";
								PJ.topdialog(iframeId, '按件号nsn号查询 ', '<%=path%>/stock/search/listByNsnPart?cageCode='+cageCode,
