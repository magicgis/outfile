...
<script type="text/javascript">
	$(function(){
		...
		//件号类型
	 	...
	 	$.ajax({
			type: "POST",
			dataType: "json",
			url:'<%=path%>/stock/search/findWeightUnit',
			success:function(result){
				var obj = eval(result.message)[0];
				if(result.success){
					for(var i in obj){
	 	...
	 	$.ajax({
			type: "POST",
			dataType: "json",
			url:'<%=path%>/stock/search/findDimentionsUnit?',
			success:function(result){
				var obj = eval(result.message)[0];
				if(result.success){
					for(var i in obj){
	...
	function validate(){
		return validate2({
			nodeName:"msn,msnFlag,partNum,partName,cageCode",
			form:"#addForm"
		});
	};
...
<div position="center" title="新增商品库">
	<form id="addForm" action="">
	<input name="bsn" id="bsn" class="hide"  value="${tPart.bsn }"/>
	<table>
		<tr>
			<td>Part Number</td>
			<td><input id="partNum" name="partNum" class="text"  value="${tPart.partNum }"/>
			</td>
		</tr>
		<tr>
			<td>Description</td>
		...
		<tr>
			<td>MSN</td>
			<td><input name="msn" id="msn" class="text"  value="${tPart.msn }"/></td>
		</tr>
		<tr>
			<td>NSN</td>
