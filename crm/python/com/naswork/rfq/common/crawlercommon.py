...
import random
import traceback
from utils import RequestHandler, Logging, Audit, OneTimeThread,MySqlProxy,StoppableThread
class CrawlerCommon(object):
    ...
    @staticmethod
    def readRawContent(url, timeout=3):
        conn = CrawlerCommon.getConnWithHeaders(url, False, None, timeout)
        return conn.read(),''
        
    @staticmethod
    def readContent(url, withHeaders=False, data=None, timeout=3):
        conn = CrawlerCommon.getConnWithHeaders(url, withHeaders, data, timeout)
        html = conn.read()
        pattern="charset=.*?\""
    ...
    def initUrlLib():
        ...
        return cj
    @staticmethod
    def getConnWithHeaders(url, withHeaders=True, data=None, timeout=3):
        if withHeaders is False:
            conn = urllib2.urlopen(url,data,timeout)            
        else:
...
class CrawlerManager(RequestHandler):
    ...
    def __startWorkers(self, begin, end, isInit=False):
        for i in range(begin, end):
            dbProxy = None
            if self._noDb is False:
                dbProxy = MySqlProxy(self._dbHost, self._dbPort, self._dbUser, self._dbPasswd, self._dbName, logger=self.logger)
...
class CrawlerWorker(RequestHandler):
    def __init__(self, controller, identifier, dbProxy, logger):
        super(CrawlerWorker, self).__init__(sleep_time=0)
    ...
    def randomSleep(self):
        if self.__sleep_range[0]==0 and self.__sleep_range[1]==0:
            return
...
class CrawlerBase(StoppableThread):
    ...
    def _fetchRawContent(self, url, timeout=30):
        try:
            (content, warning) = CrawlerCommon.readRawContent(url, timeout)
            return content
        except:
            return None
    def _fetchContent(self, url, data=None, record=True, timeout=30):
        try:
            self.totalPage+=1
            (content, warning) = CrawlerCommon.readContent(url, data=data, timeout=timeout)
