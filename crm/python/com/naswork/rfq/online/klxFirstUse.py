...
def beforehandle(searchxhr, part_num,id,elementId):
    ...
    if len(erro) > 0:
        ...
        if erromsg.find('This part is not recognized') >= 0:
            ...
            td = tr.findAll('td')
            sinventoryValue = (td[8].text).replace(',','')
...
#                 print '(recordfail)'+e1
            ...
            ''' after position 4 '''
            search_handle(soup, part_num, info, id, elementId)
        elif erromsg.find('<br  />') >= 0:
            search_handle(soup, part_num, info, id, elementId)
        else:
            print '(other position)' + part_num
...
def search_handle(searchxhr, part_num, INFORMATION,id, elementId):
    ...
    if len(res1)==0 or len(res1[-1].findAll('td')) < 10:
        ...
        while i < lens:
            ...
            text = lis[i].text
            area = ''.join(filter(lambda x:x.isalpha(), text))
            num = (re.findall("\d+",text))[0]
            quantitystr = quantitystr + area +'(' + num +')' + ','
            i += 1
        
        STOCK_MESSAGE = quantitystr[:-1]
        save_detail(part_num, DESCRIPTION, UNIT_PRICE, UNIT, CURRENCY, CAGE_CODE, STOCK_MESSAGE, INFORMATION,id,elementId)

def save_detail(a,b,c,d,e,f,g,h,id,elementId):
    conn = MySQLdb.connect(host="127.0.0.1",user="betterair",passwd="betterair",db="crm",charset="utf8")
    cursor = conn.cursor()
     
    sql = "insert into klx_quote_element(PART_NUMBER, DESCRIPTION, UNIT_PRICE, UNIT, CURRENCY, CAGE_CODE, STOCK_MESSAGE, INFORMATION,KLX_QUOTE_ID,CLIENT_INQUIRY_ELEMENT_ID) values('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')"%(a,b,c,d,e,f,g,h,id,elementId)
    cursor.execute(sql)
