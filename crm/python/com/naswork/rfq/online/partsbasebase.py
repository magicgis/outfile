...
#        result = session.get(url,params, headers=searchHeaders, verify=False,timeout=90)
#        return result.text
#    except Exception,ex:
#        if retry < 1:
#            return "nothing"
...
def beforeSearch(part,retry = 3):
    try:
        ...
        params = {'employeeId':'274712',
                  'searchText':part}
        searchHeaders["Referer"] = "https://www.partsbase.com/search/cFindBasicParts"
        result = session.post(url,params, headers=searchHeaders, verify=False,timeout=90)
        return result.text
    except Exception,ex:
        if retry < 1:
            return "nothing"
...
#        searchHeaders["Referer"] = "https://www.partsbase.com/searchResults/cDisplaySearchResultsNew?referenceNumber=None&countryID=0_&searchType=Parts&itemDesc="+part+"&searchMethod=BASIC&resultsPerPage=200&partNum=1&currentItem="+part
