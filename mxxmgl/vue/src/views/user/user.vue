<template>
  <div class="app-container">
    ...
    <el-table :data="list" v-loading.body="listLoading" element-loading-text="拼命加载中" border fit
      ...
      <el-table-column align="center" label="性别" prop="sex" style="width: 30px;" width="100">
        <template slot-scope="scope">
          <el-tag type="primary" v-text="" v-if="scope.row.sex==='1'">男</el-tag>
          <el-tag type="danger" v-text="" v-else>女</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="最近修改时间" prop="updateTime" width="170">
        <template scope = "scope">
          {{pTime(scope.row.updateTime)}}
        </template>
      </el-table-column>
      <el-table-column align="left" label="管理" width="220" v-if="hasPerm('user:update')">
        <template slot-scope="scope">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      createUser() {
        this.tempUser.password = md5(this.tempUser.password);
        //添加新用户
        this.api({
          url: "/user/insertUser",
          method: "post",
